...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'nature':
				dialogue = CoolUtil.coolTextFile(Paths.txt('nature/natureDialogue'));
			case 'tempting':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tempting/temptingDialogue'));
			case 'deam natura':
				dialogue = CoolUtil.coolTextFile(Paths.txt('deam natura/deam naturaDialogue'));
				endCut = true;
			case 'nature old':
				dialogue = CoolUtil.coolTextFile(Paths.txt('nature old/nature oldDialogue'));
			case 'tempting old':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tempting old/tempting oldDialogue'));
			case 'deam natura old':
				dialogue = CoolUtil.coolTextFile(Paths.txt('deam natura old/deam natura oldDialogue'));
		}
		...
		//adding the backpack up here so it layers behind lav
		bp = new FlxSprite().loadGraphic(Paths.image('lav/backpack'));
		bp.x = -1000;
		bp.y = -1000;
		add(bp);
		...
		if (SONG.song.toLowerCase() == 'deam natura' || SONG.song.toLowerCase() == 'deam natura old')
		{
			remove(lavTrail);
			lavTrail = new FlxTrail(dad, null, 4, 5, 0.3, 0.0015);
			add(lavTrail);
		}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'lavStage':
				boyfriend.setPosition(1492.3, 603.3);
				gf.setPosition(927.4, 282.7);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			case ('lavfinal'):
				dad.x = 880 + Math.cos(rotTime / 50 * rotSpd) * 80 * rotXLen;
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'senpai-angry':
						...
						camFollow.x = dad.getMidpoint().x - 100;
					case 'lavfinal':
						camFollow.x = dad.x + 200;
						camFollow.y = dad.y - 20;
				}
		...
		if (curSong.toLowerCase() == 'tempting')
		{
			var lavcutBeat = 184;
			if (curBeat == lavcutBeat)
			{
				if (dad.lavState == 0)
				{
					dad.lavState = 1;
					...
					{
						FlxG.sound.play(Paths.sound('bpDrop'));
					});
				}
			}
			else if (curBeat == lavcutBeat + 12)
			{
				var dX = dad.x;
				var dY = dad.y;
				var dS = dad.lavState;
				if (dS == 2)
				{
					...
					remove(dad);
					dad.destroy();
					dad = new Character(dX, dY, 'lavnobp');
					add(dad);
				}
			}
			if (dad.lavState == 2)
		...
		if (showEpil)
		{
			...
			{
				case 0:
					epilImg = new FlxSprite(0, 0, Paths.image("lav/e/0final"));
					epilImg.antialiasing = true;
					epilImg.alpha = 0;
					epilImg.scrollFactor.set();
					var fuc = 1.27;
					epilImg.setGraphicSize(Std.int(FlxG.width * fuc), Std.int(FlxG.height * fuc));
					add(epilImg);
